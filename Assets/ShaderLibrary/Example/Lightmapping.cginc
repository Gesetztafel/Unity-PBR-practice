...
struct surfaceData {
	...
	float alpha, metallic, occlusion, roughness;
};

float4 GESETZ_LightmappingFragmentProgram (Interpolators i) : SV_TARGET {
	surfaceData surface;
	surface.normal = float3(0, 1, 0);
	surface.albedo = 1;
	surface.alpha = 1;
	...
	surface.roughness = 0.5;
		...
		surface.emission = GetEmission(i);
		surface.metallic = GetMetallic(i);
		surface.roughness = GetRoughness(i);
	#endif
	...
	float roughness =surface.roughness * 0.5;
	surfaceData.Albedo += surfaceData.SpecularColor * roughness;
